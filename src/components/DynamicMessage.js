  ...
  return `
    <section class="dynamic-panel ${!isValid ? 'dynamic-invalid' : ''}" aria-labelledby="dynamic-title">
      ...
      <div data-copy-text="dynamic-text">
        <p>Com <strong>${customers} clientes recorrentes</strong> e mensalidade média de <strong>${fee}</strong> paga pelos assinantes ao seu provedor, se você tiver <strong>${beforeRate}</strong> de atrasos${isPadraoMercado ? ' (padrão do mercado de provedores)' : ''}, em um cenário com a 123zap esse número pode cair para <strong>${afterRate}</strong>:</p>
        <ul><li>Você pode liberar aproximadamente <strong>${improvement}</strong> por mês em valores que hoje ficam em atraso</li><li>Reduzir cerca de <strong>${reduction}</strong> contatos no suporte por mês</li><li>Porque menos clientes irão acionar o suporte por cobrança, atraso ou bloqueio</li></ul>
